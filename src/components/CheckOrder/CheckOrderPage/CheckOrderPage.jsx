...
export default function CheckOrderPage() {
  const { selectedTrain, trainId, trainIdBack } = useSelector(
    (state) => state.train,
  );
  const {
    ticketPricesThere,
  ...
  const confirm = (e) => {
    ...
    const dataThere = passengersInfo.map((passenger) => {
      ...
      const info = {
        ...
        document_type:
          passenger.type === 'adult' ? 'Паспорт' : 'Свидетельство о рождении',
        document_data:
          passenger.type === 'adult'
            ? passenger.series + passenger.number
            : passenger.numberChild,
      };
            ...
            selectedPlacesBack.map((seat) => {
              ticket = {
                ...
                include_children_seat:
                  passenger.type === 'adult' ? false : true,
              };
            });
            return ticket;
          })
    ...
    const order = JSON.stringify({
      user: {
        ...
        payment_method:
          personalData.payment === 'onlinePayment' ? 'online' : 'cash',
      },
      departure: {
      ...
      <div className="check__order__box">
          ...
          <div className="passsengers__info__box">
            <div className="passsengers__info__list">
              {passengersInfo.map((el, index) => (
                <AddPassengersInfo el={el} index={index} key={index} />
              ))}
            </div>
            <div className="total__cost__info">
              <div className="total__cost__text">
                Всего
        ...
        <section className="payment__method__info">
          ...
          <div className="payment__method">
            {personalData?.payment === 'onlinePayment' ? 'Онлайн' : 'Наличными'}
          </div>
          <button className="change__btn" onClick={() => navigete('/payment')}>
            Изменить
          </button>
        </section>

        <button className="confirm__btn" onClick={(e) => confirm(e)}>
          Подтвердить
        </button>
      </div>
    </>
