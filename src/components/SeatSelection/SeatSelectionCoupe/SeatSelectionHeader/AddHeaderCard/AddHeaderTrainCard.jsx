                ...
                <div className={`train__information_box train__information_box-${type}`} >
                    ...
                    <div className='train__direction__there'>
                        <div className='train__direction__start'>
                            <div className='train__direction__time'>{convertingSecondsHours(selectedTrain.from.datetime)}</div>
                            <div className='train__direction__city'>{selectedTrain.from.city.name}</div>
                            <div className='train__direction__station'>{selectedTrain.from.railway_station_name} вокзал</div>
                        </div>
                            <img className='train__direction__img' src={arroWrigth} alt='' ></img>
                        <div className='train__direction__end'>
                        <div className='train__direction__time'>{convertingSecondsHours(selectedTrain.to.datetime)}</div>
                            <div className='train__direction__city'>{selectedTrain.to.city.name}</div>
                            <div className='train__direction__station'>{selectedTrain.to.railway_station_name} вокзал</div>
                        </div>
                    </div>
                    <div className='seat__duration'>
                        ...
                        <div className='seat__duration__time'>
                            <div className='seat__duration__time_watch'>{
                                '02,03,04,05,06,22,23,24'.includes(timeDuration[0])?( timeDuration[0]+' часа') :
                                ('01,21'.includes(timeDuration[0]) ? timeDuration[0]+' час' : timeDuration[0]+' часов')}</div>
                            <div className='seat__duration__time_minutes'> {
                             '1, 21,31,41,51'.includes(timeDuration[1])? (timeDuration[1] + ' минута') : 
                            ('2,3,4,22,23,24,32,33,34,42,43,44,52,53,54'.includes(timeDuration[1]) )? timeDuration[1] + ' минуты' :
                            timeDuration[1]+' минут'}</div>
                        </div>
                ...
                <div className='number__tickets' >
                    ...
                    <div className='number__tickets__menu'>
                        <div className='number__tickets__type number__tickets__type_adult'>
                            ...
                            {( Number(type === 'there' ? seatsThere[0].count : seatsBack[0].count) === 3 ) &&   <label htmlFor='inputTypeTicketAdult' className='number__tickets__adult__input__label' >Можно добавить еще 1 пассажира</label> }
                            {( Number(type === 'there' ? seatsThere[0].count : seatsBack[0].count) >= 4 ) &&   <label htmlFor='inputTypeTicketAdult' className='number__tickets__adult__input__label' >Можно добавить не более 4 пассажиров</label> }
                        </div>
