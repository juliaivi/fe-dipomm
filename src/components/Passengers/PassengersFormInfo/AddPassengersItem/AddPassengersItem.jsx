...
export default function AddPassengersItem({ index, id, el }) {
  ...
  const [numberChildValue, setNumberChildValue] = useState({
    number: el.numberChild,
    error: false,
    valid: false,
  });
  ...
  const [twoSurname, setTwoSurname] = useState({
    name: el.twoSurname,
    error: false,
    valid: false,
  });
  ...
  const [mobilityInfo, setMobilityInfo] = useState({
    value: el.mobilityInfo,
    valid: true,
  });
  ...
  const regCertificate =
    /^M{0,3}(CM|CD|D?C{0,3})(XC|XL|L?X{0,3})(IX|IV|V?I{0,3})-[А-Яа-я\- ]{2}-\d{6}$/gm;
  ...
  return (
    <form className="passengers__forms__item" key={index}>
      <Accordion
        className="accordion__passengers__forms"
        defaultActiveKey="0"
        alwaysOpen
      >
        <Accordion.Item
          eventKey="0"
          className="accordion__passengers__forms__item"
        >
          ...
          <Accordion.Body className="accordion__passengers__forms__body">
            ...
            <div className="passengers__detailed__info">
              <div className="gender__type">
                ...
                <button
                  className={`male gender__btn ${genderType.genderType === 'M' ? 'active' : ''}`}
                  onClick={(e) => changeGender(e)}
                >
                  M
                </button>
                <button
                  className={`female gender__btn ${genderType.genderType === 'Ж' ? 'active' : ''}`}
                  onClick={(e) => changeGender(e)}
                >
                  Ж
                </button>
              ...
              <label htmlFor="dateBirth" className="date__birth__label">
                ...
                <input
                  className={`date__birth ${dateBirth.error === true ? 'error' : ''}`}
                  id="dateBirth"
                  placeholder="ДД/ММ/ГГ"
                  defaultValue={dateBirth.date}
                  onClick={() => {
            ...
            <div className="detailed__info__mobility checkbox">
              ...
              <label
                ...
                onClick={() => {
                  setMobilityInfo({
                    value: mobilityInfo.value === true ? false : true,
                    valid: false,
                  });
                  setValidForm(false);
                }}
              >
                ограничения подвижность
              </label>
            </div>
            ...
            <div className="passengers__document__info">
              <div className="passengers__document__item">
                <div className="passengers__document">
                  <div className="passengers__document__title">
                    Тип документа
                  </div>
                  <Dropdown
                ...
                {selectedDocument.name === 'Паспорт РФ' ? (
                  <>
                    <label
                      htmlFor="series"
                      className={`passengers__document__label passengers__document__${selectedDocument.type}`}
                    >
                      <div className="passengers__document__title passengers__document__title__series">
                        Серия
                      </div>
                      <input
                        type="number"
                    ...
                    <label
                      ...
                      className={`passengers__document__label passengers__document__${selectedDocument.type}`}
                    >
                      <div className="passengers__document__title passengers__document__title__number">
                        Номер
                      </div>
                      <input
                        type="number"
                  ...
                  >
                    ...
                    />
                    {!numberChildValue && (
                      <span className="passengers__document__text ">
                        12 символов
                      </span>
                    )}
                  </label>
                ...
                <>
                  ...
                  {(seriesValue.error === true ||
                    ...
                    dateBirth.error !== true && (
                      <div className="error__text">
                        {' '}
                        Заполните поле! Серия состоит из 4 цифр, а номер из 6.
                      </div>
                    )}
                  {numberChildValue.error === true &&
                    examination === false &&
