...
export default function PassengersFormInfo() {
    const {passengersInfo, seatsThere} = useSelector(state => state.passengers);
    ...
    const AddPassengers = ( e, id) =>{
        ...
        if ((Number(seatsThere[0].count) + Number(seatsThere[1].count) + Number(seatsThere[2].count)) === passengersInfo.length) {
            valid = passengersInfo.some((el) => el.validForm === false)
        } 

    return ( 
            <section className="passengers__forms__box">
                {passengersInfo.length !== 0 && passengersInfo.map((el, index) => (
                        <AddPassengersItem index={el.id} id={index} el={el}/>
                    ))
                }
 
                <div className='add_passengers__box' >
                    <div className='add_passengers__title'>Добавить пассажиров</div>
                    <button className='add_passengers' onClick={(e) => AddPassengers( e,index)}>+</button>
                </div>
              <button className={`further-btn ${valid === true ? 'disabled': '' }`} onClick={() => {  navigete('/payment')}} disabled={valid === true ? true: false  } >Далее</button>
            </section>
    )
}
